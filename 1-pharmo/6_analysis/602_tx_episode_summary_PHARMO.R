...
rx_indexed <- readRDS(file2read)


file2read <- paste0("./2_interim_data/tx_risk_factors_pg",
                    gap, ".rds")
tx_risk_factors <- readRDS(file2read)


file2read <- paste0("./2_interim_data/tx_indications_pg",
                    gap, ".rds")
tx_indications <- readRDS(file2read)

tx_info <- paste0("./2_interim_data/tx_info_pg",
                    gap,
                    ".rds") %>%
  readRDS()
...
temp <- tx_info %>%
  # head(5) %>% 
  ungroup() %>%
  mutate(patid = factor(gsub("-.", "", tx_episode))) %>% 
  group_by(patid) %>% 
  summarise(n = n())
...
temp <- "incident" %>% 
  as_tibble() %>% 
  rename(rx_type = value)

rx1 <- rx_indexed %>% 
  semi_join(temp, by = "rx_type") %>% 
  ungroup() %>% 
  group_by(atc_code) %>% 
  summarise(n = n()) %>% 
...
s3_indications <- s3 %>% 
  ...
  mutate(description = if_else(description == "UKNOWN", "Uknown", description),
         main = TRUE) 
